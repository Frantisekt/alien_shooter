...
def update_screen(ai_settings, screen, stats, sb, ship, aliens, 
	...
	# Redraws all bullets behind ships and aliens.
...
def fire_bullet(ai_settings, screen, ship, bullets):
	"""Fires bullet if limit not reached yet."""
	if len(bullets) < ai_settings.bullets_allowed:
		new_bullet = Bullet(ai_settings, screen, ship)
		bullets.add(new_bullet)

def get_number_aliens_x(ai_settings, alien_width):
	"""Determines the number of aliens that fits in a row"""
 ...
 aliens, bullets):
	...
	for alien in aliens.sprites():
		if alien.rect.bottom >= screen_rect.bottom:
			# Treats thisthe same as if the ship got hit.
